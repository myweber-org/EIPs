...
class WeatherFetcher:
    ...
    def get_weather_summary(self, city: str) -> Optional[str]:
        ...
        source_note = " (from cache)" if weather_data['source'] == 'cache' else ""
        
        summary = (
            f"Weather in {weather_data['city']}, {weather_data['country']}{source_note}:\n"
            ...
            f"Humidity: {weather_data['humidity']}% | Pressure: {weather_data['pressure']} hPa\n"
...
def main():
    ...
    for city in cities:
        print("\n" + "="*50)
        summary = fetcher.get_weather_summary(city)
        print(summary)
        time.sleep(1)
