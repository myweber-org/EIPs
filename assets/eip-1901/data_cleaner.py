...
class DataCleaner:
    ...
    def impute_missing(self, strategy='mean', custom_value=None):
...
def example_usage():
    ...
    cleaned_df = (cleaner
                 .remove_outliers(['A', 'B'], method='iqr')
...
if __name__ == "__main__":
    ...
    print(f"Final dataset shape: {result.shape}")import pandas as pd
...
if __name__ == "__main__":
    ...
    is_valid, msg = validate_data(cleaned, required_columns=['A', 'B'], min_rows=3)
    print(f"\nValidation: {msg}")
import numpy as np
import pandas as pd
...
def clean_dataset(df, outlier_threshold=1.5, normalize_method='minmax', missing_strategy='mean'):
    ...
    print(f"Normalized data using {normalize_method} method")
