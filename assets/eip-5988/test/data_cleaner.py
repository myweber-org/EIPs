...
def validate_dataframe(df):
    ...
    return len(errors) == 0, errorsimport numpy as np
import pandas as pd
...
def validate_cleaning(df_before, df_after, numeric_columns):
    report = {}
    for col in numeric_columns:
        if col in df_before.columns and col in df_after.columns:
            report[col] = {
                'original_mean': float(df_before[col].mean()),
                'cleaned_mean': float(df_after[col].mean()),
                'original_std': float(df_before[col].std()),
                'cleaned_std': float(df_after[col].std()),
