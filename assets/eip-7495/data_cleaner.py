...
def normalize_column(data, column, method='minmax'):
    ...
    return data_copyimport pandas as pd
import numpy as np
...
def validate_dataset(df, required_columns=None, min_rows=1):
    ...
    if df.isnull().all().any():
        ...
        return False, f"Columns completely empty: {empty_cols}"
...
if __name__ == "__main__":
    sample_data = {
        'id': [1, 2, 3, 4, 5, 5, 6],
        'value': [10.5, 20.3, None, 40.7, 50.1, 50.1, 1000.0],
    ...
    print(f"\nAfter outlier removal: {len(filtered)} rows")