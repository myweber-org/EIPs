...
if __name__ == "__main__":
    ...
    clean_csv(input_file, output_file)import numpy as np
import pandas as pd
...
def validate_dataframe(df, required_columns=None):
    ...
    if df.isnull().any().any():
        ...
        validation_result['warnings'].append(f'Columns with missing values: {null_columns}')
...
def clean_dataset(df, numeric_columns, outlier_method='iqr', normalize_method='minmax', missing_strategy='mean'):
    ...
    df_cleaned = handle_missing_values(df_cleaned, strategy=missing_strategy, columns=numeric_columns)
...
def create_sample_data():
    ...
    df.loc[np.random.choice(df.index, 50), 'feature_a'] = np.random.uniform(300, 500, 50)
    df.loc[np.random.choice(df.index, 30), 'feature_b'] = np.random.uniform(400, 600, 30)
