        ...
        for col, expected_type in dtypes.items():
            if col in data.columns:
                if not pd.api.types.is_dtype_equal(data[col].dtype, expected_type):
                    return False, f"Column {col} has wrong dtype. Expected {expected_type}, got {data[col].dtype}"
...
def create_sample_data():
    ...
    for idx in outlier_indices:
        col = np.random.choice(['feature_a', 'feature_b'])
        df.at[idx, col] = df.at[idx, col] * 10
...
def clean_dataset(df, missing_threshold=0.5, normalize=True, encode_cats=True):
    """
...
if __name__ == "__main__":
    sample_data = {
        'values': [10, 12, 12, 13, 14, 15, 15, 16, 17, 18, 100, 120, -10]
    }
    df = pd.DataFrame(sample_data)
...
if __name__ == "__main__":
    ...
    sample_data = {
        ...
        'age': [25, 30, 25, 35, 40],
        'email': ['ALICE@example.com', 'bob@test.com', 'alice@example.com', 'CHARLIE@demo.org', 'dave@sample.net']
    }
