...
def clean_numeric_columns(df, columns, fill_method='mean'):
    ...
    Parameters:
    df (pd.DataFrame): Input DataFrame
    columns (list): List of numeric column names to clean
    ...
    for col in columns:
        ...
        if missing_count > 0:
            ...
            print(f"Filled {missing_count} missing value(s) in column '{col}' with {fill_method}")
...
def validate_dataframe(df, required_columns=None, min_rows=1):
    ...
    if len(df) < min_rows:
        print(f"Error: DataFrame must have at least {min_rows} row(s)")
        return False
...
if __name__ == "__main__":
    sample_data = {
        ...
        'score': [85.5, 92.0, 92.0, 78.5, None, 88.0, 95.5],
    ...
    cleaned_df = clean_numeric_columns(cleaned_df, ['score', 'age'], fill_method='mean')
...
def clean_dataset(df, columns_to_check=None, drop_duplicates=True, fill_missing='mean'):
    ...
    Parameters:
    df (pd.DataFrame): Input DataFrame
    columns_to_check (list): List of columns to check for duplicates, None for all columns
    drop_duplicates (bool): Whether to drop duplicate rows
...
def example_usage():
    ...
    is_valid, message = validate_data(cleaned_df, required_columns=['id', 'value'], min_rows=3)
