...
def validate_dataframe(df, required_columns=None):
    ...
    validation_results = {
        ...
        'columns': list(df.columns) if isinstance(df, pd.DataFrame) else None,
        'missing_values': df.isnull().sum().sum() if isinstance(df, pd.DataFrame) else None,
        'duplicates': df.duplicated().sum() if isinstance(df, pd.DataFrame) else None
    }
